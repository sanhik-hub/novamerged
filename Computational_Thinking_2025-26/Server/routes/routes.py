from services.workspace_services import (
    ...
    get_pending_workspace_sent_invitations,
...
from services.workspace_solution_services import (
    ...
    exit_workspace_question_attempt,
    terminate_workspace_question_attempt,
...
def register_routes(app):
    ...
    @app.route("/WorkspaceSentInvitations", methods=["POST"])
    def receive_WorkspaceSentInvitations():
        user, auth_error = get_authenticated_user()
        ...
        invitations, error = get_pending_workspace_sent_invitations(
            user.username,
            workspace_id,
        )
        ...
        return {
            "message": "Workspace sent invitations retrieved successfully",
            "data": invitations,
        }, 200
    ...
    @app.route("/WorkspaceRemoveMember", methods=["POST"])
    def workspace_remove_member():
        ...
        if not isinstance(member_username, str):
            return {
                "error": "MemberUsername must be a string"
            }, 400

        result, error = remove_workspace_member(
    ...
    @app.route("/WorkspaceTransferOwnership", methods=["POST"])
    def workspace_transfer_ownership():
        user, auth_error = get_authenticated_user()
        ...
        result, error = transfer_workspace_ownership(
    ...
    @app.route("/WorkspaceMemberPerformance", methods=["POST"])
    def workspace_member_performance():
        data = request.get_json(silent=True)
        ...
        if "WorkspaceId" not in data or "UserId" not in data:
            return {
        ...
        if (
            isinstance(member_user_id, bool)
            or not isinstance(member_user_id, int)
        ):
            return {"error": "UserId must be an integer"}, 400
        ...
        result, error = get_workspace_member_performance(
            username=user.username,
            workspace_id=workspace_id,
        ...
        if error in {
            "You are not a member of this workspace",
            "Only workspace admins can view member performance",
        }:
            return {"error": error}, 403
    ...
    def workspace_question_compute():
        ...
        # Provider selection is intentionally controlled by the server.
        # Keep the request field compatible for future provider selection,
        # but do not allow the client to override the current provider.
        provider = "gemini"
    ...
    @app.route("/WorkspaceQuestionAttemptExit", methods=["POST"])
    def workspace_question_attempt_exit():
        data = request.get_json(silent=True)
        ...
        result, error = exit_workspace_question_attempt(
            username=user.username,
            workspace_id=workspace_id,
            attempt_id=attempt_id,
        )
    ...
    @app.route("/WorkspaceQuestionAttemptTerminate", methods=["POST"])
    def workspace_question_attempt_terminate():
        data = request.get_json(silent=True)
        ...
        result, error = terminate_workspace_question_attempt(
            username=user.username,
            workspace_id=workspace_id,
            attempt_id=attempt_id,
        )
