...
def get_workspace_details(username: str, workspace_id: int):
    ...
    for member in workspace_memberships:
        member_user = User.query.filter_by(id=member.user_id).first()
...
def leave_workspace(username: str, workspace_id: int):
    ...
    if membership.role == "owner":
        return None, (
            "Workspace owners cannot leave the workspace. "
...
def get_pending_workspace_sent_invitations(
    username: str,
    workspace_id: int,
):
    ...
    try:
        ...
        return None, "Failed to retrieve sent workspace invitations"
...
def remove_workspace_member(
    ...
    if member_user.id == admin.id:
        return None, "You cannot remove yourself"

    if membership.role == "owner":
        return None, "Workspace owner cannot be removed"

    if membership.role == "admin" and admin_membership.role != "owner":
        return None, "Admins cannot remove other admins"
    ...
    try:
        ...
        db.session.rollback()
        return None, "Failed to remove workspace member"
...
def transfer_workspace_ownership(
    ...
    owner_membership = WorkspaceMember.query.filter_by(
        workspace_id=workspace_id,
        user_id=owner.id,
    ).first()
    ...
    if owner_membership.role != "owner":
        return None, "Workspace owner privileges required"
    ...
    if member_user.id == owner.id:
        return None, "You already own this workspace"

    if target_membership.role == "owner":
        return None, "User is already the workspace owner"
    ...
    try:
        ...
        return {
            ...
            "previous_owner_username": owner.username,
            "new_owner_id": member_user.id,
            "new_owner_username": member_user.username,
            "role": target_membership.role,
        }, None

    except Exception:
        db.session.rollback()
        return None, "Failed to transfer workspace ownership"
