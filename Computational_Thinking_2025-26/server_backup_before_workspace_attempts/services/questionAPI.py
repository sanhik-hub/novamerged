...
Before returning, perform a strict mathematical validation for every generated question:
- Solve the generated question yourself before writing the options.
- Put the mathematically correct result in exactly one option.
- Construct the three incorrect options from specific plausible mathematical mistakes.
- Re-check every option against the solved question.
- The value of correct_option MUST identify the one option that is mathematically correct, using 1=A, 2=B, 3=C, 4=D.
- Never return a question when zero options are correct or when more than one option is correct; regenerate that question first.
- For calculation questions, independently substitute/differentiate/integrate where applicable to verify the selected correct option.
- Hints, correct_option, options, and solutions MUST all agree.
